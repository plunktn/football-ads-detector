...
class ShotClassTests(unittest.TestCase):
    def test_sideline_frame_is_lateral(self):
        self.assertEqual(classify_shot(_sideline_frame()), ShotKind.LATERAL)
    ...
    def test_navy_frame_is_replay_or_graphic(self):
        frame = np.full((480, 640, 3), NAVY, dtype=np.uint8)
        self.assertEqual(classify_shot(frame), ShotKind.REPLAY_OR_GRAPHIC)
...
class LocateZonesTests(unittest.TestCase):
    def test_lateral_main_uses_led_roi(self):
    ...
    def test_wide_and_replay_return_empty(self):
        height, width = 720, 1280
        ...
        self.assertEqual(locate_zones(graphic, None, ShotKind.REPLAY_OR_GRAPHIC), [])
    ...
    def test_other_positions_are_stubs(self):
        ...
        located = locate_zones(frame, profile, ShotKind.LATERAL)
    ...
    def test_far_led_sideline_still_locates(self):
        ...
        shot = classify_shot(frame)
        self.assertEqual(shot, ShotKind.LATERAL)
        located = locate_zones(frame, None, shot)
        self.assertGreaterEqual(len(located), 1)
        self.assertFalse(located[0][1].skipped, located[0][1].reason)

    def test_elevated_shot_with_led_is_lateral(self):
        ...
        self.assertEqual(classify_shot(frame), ShotKind.LATERAL)
